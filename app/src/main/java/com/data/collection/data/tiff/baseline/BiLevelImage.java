...
import com.data.collection.data.tiff.BaseLineImage;
import com.data.collection.data.utils.LogCat;
...
public class BiLevelImage extends BaseLineImage {
	protected  LogCat log = new LogCat();
...
//		image = new BufferedImage(BufferedImage.TYPE_BYTE_BINARY, (int) imageLength, (int) imageWidth, null);
...
//			short[][]	pixel = new short[(int) imageWidth][(int)imageLength]; //allocate the 2d pixel array; 
		...
		for (int offset = 0; offset < stripOffsets.length; offset++) {
			...
			int count_max = (int) stripByteCounts[offset];// how much bytes we have to read from above offset i.e size
															// of strip
	...
	public Rectangle getBounds() {
		...
		rect.setBounds(0,0,(int) imageLength, (int) imageWidth);
		return rect;
	}
